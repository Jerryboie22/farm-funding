...
const ARTICLE_TAGS: Record<string, string[]> = {
  ...
  "260407ForestProductsOutlook": ["ag economy", "outlook", "forestry", "timber"],
...
/* Reference pages do not all use the same closing modules. */
  ...
  return (
    <main className="w-full bg-white text-charcoal">
      <header className="mx-auto w-full max-w-[1180px] px-6 pb-8 pt-10 md:px-10 md:pt-12 lg:px-12 lg:pt-14">
        <time dateTime={article.date} className="block font-body text-[15px] leading-6 text-grey-text">
          {formatDate(article.date)}
        </time>
        <Link href={`${BLOG_PATH}?category=${encodeURIComponent(article.category)}`} className="mt-1 block font-display text-[15px] font-bold leading-6 text-clay hover:underline">
          {article.category}
        </Link>
      ...
      <article className="mx-auto w-full max-w-[850px] px-6 pb-10 pt-9 md:px-10 md:pt-11 lg:px-0 lg:pt-12">
        ...
        {showCta && (
          <div className="mt-9">
            <Link href="/contact" className="inline-flex bg-clay px-7 py-3 font-display text-sm font-bold text-white hover:bg-clay-dark">
              Contact Us Today!
            </Link>
          </div>
        )}
        ...
        <div className="mt-8 border-t border-charcoal/10 pt-5">
          ...
          {tags.map((tag, index) => (
            <span key={tag}>
              ...
              {index < tags.length - 1 && <span className="mx-2 text-charcoal">,</span>}
            </span>
          ))}
        </div>
      ...
      <section className="w-full bg-cream px-6 py-10 md:py-13">
          ...
          <div className="mt-6 grid gap-6 md:grid-cols-3">
            {related.map((item) => (
              <Link key={item.slug} href={`${BLOG_PATH}/${item.slug}`} className="group bg-white">
                ...
                <div className="p-5">
                  <p className="font-display text-xs font-bold text-clay">{item.category}</p>
                  <h3 className="mt-2 font-display text-[19px] font-bold leading-[1.25] text-forest group-hover:underline">{item.title}</h3>
                  <p className="mt-2 font-body text-xs text-grey-text">{formatDate(item.date)}</p>
                </div>
              </Link>
            ))}
          </div>
