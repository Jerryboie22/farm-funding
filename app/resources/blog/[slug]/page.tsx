...
function authorRole(author: string) {
  ...
  if (author === "Dario Arezzo") return "Executive Vice President and Chief Financial Services Officer";
...
export default async function HarvestArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="w-full bg-white text-charcoal">
      {/* Reference article header: date → category → title → byline → hero image */}
      <section className="w-full bg-white">
        <div className="mx-auto w-full max-w-[1180px] px-[4%] pt-12 md:pt-14 lg:pt-16">
          <time
            dateTime={article.date}
            className="font-body text-[15px] leading-6 text-grey-text"
          >
      ...
      {/* Article writing area. Existing article text is rendered without rewriting it. */}
      <section className="w-full bg-white">
        <div className="mx-auto w-full max-w-[900px] px-[6%] py-12 md:py-16 lg:py-20">
          <article>
            {article.sections.map((section, index) => (
              <section key={`${section.heading}-${index}`} className="mb-10 md:mb-12 last:mb-0">
                <h2 className="font-display text-[26px] leading-[1.25] font-bold text-forest md:text-[30px]">
                  {section.heading}
                </h2>
                <div className="mt-4 whitespace-pre-line font-body text-[17px] leading-[1.8] text-charcoal md:text-[18px]">
                  {section.body}
                </div>
              </section>
            ))}
          ...
          <div className="mt-12 border-t border-charcoal/10 pt-6">
            <p className="font-body text-sm leading-6 text-grey-text">
              Tags: <span className="text-clay">{article.category.toLowerCase()}</span>, <span className="text-clay">today&apos;s harvest</span>
            </p>
          </div>

          {/* Author block */}
          <div className="mt-8 flex items-start gap-5 border-t border-charcoal/10 pt-8">
            <div className="h-20 w-20 shrink-0 overflow-hidden rounded-full bg-grey-bg" aria-hidden="true" />
            <div>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-4 border-t border-charcoal/10 pt-6">
            ...
            <a href="#facebook" aria-label="Share on Facebook" className="font-display text-sm font-bold text-clay hover:underline">Facebook</a>
            <a href="#x" aria-label="Share on X" className="font-display text-sm font-bold text-clay hover:underline">X</a>
            <a href="#linkedin" aria-label="Share on LinkedIn" className="font-display text-sm font-bold text-clay hover:underline">LinkedIn</a>
            <a href={`mailto:?subject=${encodeURIComponent(article.title)}`} aria-label="Share by email" className="font-display text-sm font-bold text-clay hover:underline">Email</a>
          </div>
        </div>
      </section>

      {/* Reference related-article block */}
      <section className="w-full bg-white">
          ...
          <div className="mt-8 grid grid-cols-1 gap-7 md:grid-cols-3">
            {related.map((item) => (
              <article key={item.slug} className="min-w-0">
                ...
                <div className="pt-5">
                  <span className="font-display text-xs font-bold uppercase tracking-wide text-grey-text">
                    {item.category}
                  </span>
                  <Link href={`${BLOG_PATH}/${item.slug}`}>
                    <h3 className="mt-2 font-display text-[20px] leading-7 font-semibold text-gold hover:underline">
                      {item.title}
                    </h3>
                  </Link>
                  <p className="mt-3 font-body text-sm leading-6 text-grey-text">
      ...
      {/* Reference closing sections */}
      <section className="w-full bg-forest-dark px-[4%] py-14 text-center text-white md:py-16">
        <div className="mx-auto max-w-2xl">
          <h2 className="font-display text-[30px] leading-[1.2] font-bold text-white md:text-[36px]">
            Meet the Authors
          </h2>
          ...
          <Link
            href="/contact"
            className="mt-7 inline-flex border-2 border-white bg-transparent px-6 py-3 font-display text-sm font-bold text-white hover:bg-white hover:text-forest-dark"
          >
            Meet the Authors
          </Link>
        </div>
      ...
      <section className="w-full bg-cream px-[4%] py-12 text-center md:py-16">
        <div className="mx-auto max-w-2xl">
          ...
          <p className="mt-4 font-body text-base leading-7 text-charcoal/80">
          ...
          <form className="mx-auto mt-6 flex max-w-[560px] flex-col gap-3 sm:flex-row">
            ...
            <input
              ...
              className="min-h-12 flex-1 border border-charcoal/20 bg-white px-4 py-3 text-sm text-charcoal placeholder:text-grey-text outline-none"
            />
            <button
              type="submit"
              className="min-h-12 bg-clay px-6 py-3 font-display text-sm font-bold text-white hover:bg-clay-dark"
            >
              Sign Up
            </button>
          </form>
