...
export default async function HarvestArticlePage({
  ...
  return (
    <main className="w-full bg-white text-charcoal">
      {/* Article hero — same visual system as the Today's Harvest reference page */}
      <section className="w-full bg-cream mt-8">
        <div className="grid grid-cols-1 md:grid-cols-12 px-[4%]">
          <div className="md:col-span-6 flex flex-col justify-center py-12 md:py-14 lg:py-20 md:pr-12 lg:pr-20">
            <Link
              href={BLOG_PATH}
              className="font-body text-sm font-bold text-grey-text hover:text-clay transition-colors"
            >
              {article.category}
            </Link>

            <h1 className="mt-4 max-w-[720px] font-display text-4xl md:text-[48px] lg:text-[54px] font-extrabold text-forest leading-[1.08]">
              {article.title}
            </h1>

            <p className="mt-6 font-body text-base text-charcoal/80 leading-relaxed">
              {article.excerpt}
            </p>
          ...
          <div className="md:col-span-6">
            <img
              src={article.image}
              alt={article.title}
              className="block w-full h-[300px] md:h-[430px] lg:h-[500px] object-cover"
            />
          </div>
        </div>
      </section>

      {/* Article body — content is preserved exactly from the article data */}
      <section className="w-full bg-white">
        <div className="mx-auto w-full max-w-[900px] px-[6%] py-12 md:py-16 lg:py-20">
          <article>
            <p className="font-body text-[17px] md:text-[18px] leading-[1.8] text-charcoal">
              {article.excerpt}
            </p>
            ...
            <div className="mt-10 md:mt-12">
              {article.sections.map((section, index) => (
                <section key={`${section.heading}-${index}`} className="mb-10 md:mb-12 last:mb-0">
                  <h2 className="font-display text-2xl md:text-[32px] font-bold text-forest leading-tight">
                    {section.heading}
                  </h2>
                  <div className="mt-4 font-body text-[17px] md:text-[18px] leading-[1.8] text-charcoal whitespace-pre-line">
                    {section.body}
                  </div>
                </section>
              ))}
      ...
      <section className="w-full bg-grey-bg">
        <div className="mx-auto max-w-[900px] px-[6%] py-8 md:py-10">
          <Link
            href={BLOG_PATH}
            className="inline-flex items-center font-display text-sm font-bold text-clay hover:text-forest transition-colors"
          >
