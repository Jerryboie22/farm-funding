...
export function generateStaticParams() { return ALL_REFERENCE_ARTICLES.map((article) => ({ slug: article.slug })); }
...
function getTags(slug: string, category: string) { return ARTICLE_TAGS[slug] ?? [category]; }
        ...
        <div className="mt-8 font-body text-[17px] leading-[1.8] text-charcoal md:text-[18px]">
          {article.sections.map((section, index) => (
            <section key={`${section.heading}-${index}`} className="mb-7 last:mb-0">
              {isNestedHeading(section.heading, slug) ? <h3 className="mb-2 font-display text-[19px] font-bold leading-[1.35] text-forest md:text-[21px]">{section.heading}</h3> : <h2 className="mb-3 font-display text-[23px] font-bold leading-[1.3] text-forest md:text-[27px]">{section.heading}</h2>}
              <p className="whitespace-pre-line">{section.body}</p>
            </section>
          ))}
        </div>

        <div className="mt-10"><Link href="/contact" className="inline-flex bg-clay px-7 py-3 font-display text-sm font-bold text-white hover:bg-clay-dark">Contact Us Today!</Link></div>

        <div className="mt-9 border-t border-charcoal/10 pt-6"><p className="font-display text-sm font-bold text-charcoal">Tags:</p><div className="mt-2 flex flex-wrap gap-x-3 gap-y-1">{tags.map((tag) => <Link key={tag} href={`${BLOG_PATH}?tag=${encodeURIComponent(tag)}`} className="font-body text-sm text-clay hover:underline">{tag}</Link>)}</div></div>

        <div className="mt-8 flex items-center gap-5 border-t border-charcoal/10 pt-7"><div className="h-20 w-20 shrink-0 overflow-hidden rounded-full bg-cream"><img src={article.image} alt="" className="h-full w-full object-cover" /></div><div><p className="font-display text-lg font-bold text-forest">{article.author}</p><p className="mt-1 font-body text-sm leading-6 text-grey-text">{authorRole}</p></div></div>

        <div className="mt-7 flex flex-wrap items-center gap-3 border-t border-charcoal/10 pt-6"><span className="mr-1 font-display text-sm font-bold text-forest">Share this post on</span><button type="button" aria-label="Share on Facebook" className="h-8 w-8 border border-charcoal/15 font-display text-sm text-forest">f</button><button type="button" aria-label="Share on LinkedIn" className="h-8 w-8 border border-charcoal/15 font-display text-sm text-forest">in</button><button type="button" aria-label="Copy article link" className="h-8 border border-charcoal/15 px-3 font-display text-xs text-forest">Copy link</button></div>
      </article>

      <section className="w-full bg-cream px-6 py-11 md:py-14"><div className="mx-auto max-w-[1180px]"><h2 className="font-display text-[28px] font-bold leading-[1.2] text-forest md:text-[34px]">You Might Also Like</h2><div className="mt-6 grid gap-6 md:grid-cols-3">{related.map((item) => <Link key={item.slug} href={`${BLOG_PATH}/${item.slug}`} className="group bg-white"><img src={item.image} alt="" className="h-[190px] w-full object-cover" /><div className="p-5"><p className="font-display text-xs font-bold text-clay">{item.category}</p><h3 className="mt-2 font-display text-[19px] font-bold leading-[1.25] text-forest group-hover:underline">{item.title}</h3><p className="mt-2 font-body text-xs text-grey-text">{formatDate(item.date)}</p></div></Link>)}</div></div></section>
