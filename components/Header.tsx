      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 bg-white overflow-y-auto">
          ...
          <div className="flex items-center justify-between px-5 py-3 border-b border-line">
            <Link
              href="/"
              className="flex items-center gap-1.5 font-display text-xl font-extrabold text-charcoal tracking-tight"
          ...
          <div className="bg-cream px-5 py-1">
            {NAV.map((section, idx) => {
              ...
              return (
                ...
                >
                  <button
                    className={`w-full flex items-center justify-between py-4 text-sm font-bold uppercase tracking-wide transition-colors ${
                  ...
                  {isOpen && (
                    <div className="border-l-2 border-slate pl-4 pb-4 flex flex-col gap-3.5">
                      {section.items.map((item) => (
                        <Link
                          key={item.href}
                          href={item.href}
                          className="text-sm font-bold text-grey-text hover:text-slate transition-colors"
                          onClick={() => setMobileOpen(false)}
                        >
                          {item.label}
                        </Link>
                      ...
                      <Link
                        href={section.href}
                        className="text-sm font-bold uppercase text-grey-text hover:text-slate transition-colors"
                        onClick={() => setMobileOpen(false)}
                      >
                        {section.seeAllLabel}
                      </Link>
            ...
            <div className="flex flex-col gap-4 py-5">
              <Link
                href="/search"
                className="flex items-center gap-3 text-sm text-grey-text"
                onClick={() => setMobileOpen(false)}
              >
                <svg
                  width="18"
            ...
            <Link
              href="/sign-in"
              className="block text-center py-3 mb-5 bg-slate text-white text-sm font-display font-bold rounded-[4px]"
              onClick={() => setMobileOpen(false)}
            >
              Sign in
            </Link>
