    ...
    <header className="sticky top-0 z-50 bg-white">
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-[60] bg-white overflow-y-auto">
          {/* Mobile menu header */}
          <div className="flex min-h-[128px] items-center justify-between px-6 border-b border-line bg-white">
            <Link
              href="/"
              className="flex items-center gap-3 font-display text-[36px] leading-none font-extrabold text-charcoal tracking-tight"
            ...
            <button
              ...
              className="flex h-12 w-12 items-center justify-center text-grey-text"
            >
              <svg
                width="42"
                height="42"
          ...
          {/* Accordion body */}
          <div className="bg-[#F0F4E7] px-[84px] pt-[32px] pb-[48px]">
            {NAV.map((section, idx) => {
              ...
              return (
                <div key={section.label}>
                  <button
                    className={`w-full flex items-center justify-between text-left uppercase transition-colors ${
                      isOpen
                        ? "text-[#4E7488]"
                        : "text-[#626466]"
                    }`}
                    onClick={() =>
                      setMobileOpenIdx(isOpen ? null : idx)
                    }
                    aria-expanded={isOpen}
                  >
                    <span className="font-display text-[36px] leading-[1.2] font-bold">
                      {section.label}
                    </span>
                  ...
                  {isOpen && (
                    <div className="mt-[38px] mb-[52px] border-l-[8px] border-[#4E7488] rounded-l-[5px] pl-[20px]">
                      <div className="flex flex-col gap-[34px]">
                        {section.items.map((item) => (
                          <Link
                            key={item.href}
                            href={item.href}
                            className="font-display text-[30px] leading-[1.35] font-bold text-[#626466] hover:text-[#4E7488] transition-colors"
                            onClick={closeMobileMenu}
                          >
                            {item.label}
                          </Link>
                        ...
                        <Link
                          href={section.href}
                          className="font-display text-[28px] leading-[1.3] font-bold uppercase text-[#626466] hover:text-[#4E7488] transition-colors"
                          onClick={closeMobileMenu}
                        >
                          {section.seeAllLabel}
                        </Link>
            ...
            <div className="h-[4px] bg-[#686868] mt-[4px] mb-[48px]" />

            {/* Utility links */}
            <div className="flex flex-col gap-[34px]">
              <Link
                href="/search"
                className="flex items-center gap-[18px] font-display text-[35px] leading-[1.3] font-normal text-[#646668]"
                onClick={closeMobileMenu}
              >
                <svg
                  width="40"
            ...
            <Link
              href="/sign-in"
              className="mt-[54px] flex h-[58px] w-[315px] items-center justify-center bg-[#4E7488] text-white font-display text-[26px] font-bold rounded-none"
              onClick={closeMobileMenu}
            >
              Sign in
            </Link>
